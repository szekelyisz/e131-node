...
function Client(arg, port) {
  ...
  if (arg instanceof Object) {
    this.host = arg.host;
    this.port = arg.port || e131.DEFAULT_PORT;
    this._socket = dgram.createSocket('udp4');
    this._socket.bind(0, arg.bind, () => {
      if (arg.mcastif !== undefined)
        this._socket.setMulticastInterface(arg.mcastif);
      if (arg.loopback === true) this._socket.setMulticastLoopback(true);
    });
  } else {
...
Client.prototype.send = function send(packet, callback) {
  ...
  this._socket.send(
    packet.getBuffer(),
    this.port,
    this.host || e131.getMulticastGroup(packet.getUniverse()),
    function onSend() {
